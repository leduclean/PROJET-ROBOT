...
// Fonction d'initialisation de l'IR
...
void Robot::line_follower() {
  ...
  if (digitalRead(LINE_FOLLOWER_LEFT_PIN) == LOW)  NewSensorState = 1;
  if (digitalRead(LINE_FOLLOWER_MID_PIN) == LOW)   NewSensorState |= 2;
  if (digitalRead(LINE_FOLLOWER_RIGHT_PIN) == LOW) NewSensorState |= 4;
